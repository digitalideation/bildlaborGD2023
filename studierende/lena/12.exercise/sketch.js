...
function draw() {
  ...
  if (webcamAngezeigt1) {
    ...
    if ((randomX + quadratGroesse) > windowWidth) {
        randomX = windowWidth - quadratGroesse;
    }
    if ((randomY + quadratGroesse) > windowHeight) {
        randomY = windowHeight - quadratGroesse;
    }
    
    image(captureCopy1, randomX, randomY, quadratGroesse, quadratGroesse);
  }
...
textDiv.addEventListener("scroll", (event) => {
